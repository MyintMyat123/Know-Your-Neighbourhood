...
class Login extends Component {
    componentDidMount() {
        ...
        if(this.props.location.state && this.props.location.state.error) {
            console.log("Login Component Dimount");
            setTimeout(() => {
...
class LoginForm extends Component {
    constructor(props) {
        ...
        this.state = {
            email: '',
            password: '',
            loginAttempts: 0, // Track the number of login attempts
            lockedOut: false,
        };
    ...
    handleSubmit(event) {
        ...
        if (this.state.lockedOut) {
            Alert.error("Account is locked. Please try again later.");
            return;
        }
        ...
        .then(response => {
            ...
            const loginAttempts = this.state.loginAttempts + 1;
            this.setState({ loginAttempts }, () => {
                if (loginAttempts >= MAX_LOGIN_ATTEMPTS) {
                    Alert.error(`Too many unsuccessful login attempts. Your account is now locked.`);
